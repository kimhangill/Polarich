<template>
    <div>
      <div class="portfolio-section">
        ...
        <div class="text-right mt-3">
          <v-btn color="deep-purple accent-4" class="white--text" @click="dialog = true">
            수정
          </v-btn>
        </div>
      </div>
      ...
      <v-dialog v-model="dialog" max-width="600px">
        <v-card>
          <v-card-title>
            <h3 class="section-title mt-3">포트폴리오 수정</h3>
          </v-card-title>
          <v-card-text>
            <PortfolioEditView :portfolio="portfolio" />
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" text @click="handleSave">저장</v-btn>
  ...
  import PortfolioEditView from '@/views/PortfolioEditView.vue'; // 수정 컴포넌트 임포트
  
  const userStore = useUserStore();
  const router = useRouter();
  const portfolio = ref({});
  const dialog = ref(false); // 다이얼로그 상태를 관리할 변수
