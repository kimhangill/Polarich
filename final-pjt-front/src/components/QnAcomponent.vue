<template>
    <v-container>
      ...
      <v-row v-for="comment in comments" :key="comment.id" class="answer-item">
        <v-col cols="12">
          <v-card class="mb-4">
            <v-card-text>
              <div class="answer-header section-title">
                <v-avatar>
              ...
              <div v-if="editingCommentId !== comment.id" class="answer-content" width="80%">
                <div v-html="renderMarkdown(comment.content)" class="ms-5"></div>
              </div>
              <div v-else>
                <v-textarea v-model="editedCommentContent" label="답변 수정" outlined></v-textarea>
      ...
      <v-row v-if="userStore.userProfile && userStore.userProfile.isProfessional">
        <v-col cols="12">
          <v-form>
            <v-textarea
              ...
              outlined
            ></v-textarea>
            <v-btn color="primary" @click="postComment">답변 작성</v-btn>
          </v-form>
        </v-col>
      </v-row>
  ...
  const refreshComments = async () => {
    try {
      ...
      console.error('답변 목록을 갱신하는 중 오류 발생:', error);
    }
  };
